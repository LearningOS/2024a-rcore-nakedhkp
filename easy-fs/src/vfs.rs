...
impl Inode {
    ...
    /// get inode id
    pub fn get_inode_id(&self, start_block: u32) -> u32{
        let inode_size = core::mem::size_of::<DiskInode>();
        let inodes_per_block = (BLOCK_SZ / inode_size) as u32; // 4
        (self.block_id as u32 - start_block) * inodes_per_block + (self.block_offset  / inode_size ) as u32
    }
    ...
    /// 通过块缓存获取索引节点所在的数据块，锁定后修改索引节点数据，并将其传递给闭包 f 进行处理。
    ...
    pub fn create(&self, name: &str) -> Option<Arc<Inode>> {
        ...
        // 将待创建的文件的目录项插入到根目录的内容中使得之后可以索引过来
